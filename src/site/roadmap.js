...
$('#pillars').innerHTML = data.pillars.map(p => `
  <div class="pillar reveal"><span class="ico">${p.emoji}</span><h3>${esc(p.title_fr)}</h3><p>${esc(p.text_fr)}</p></div>`).join('');

const used = [...new Set(data.releases.flatMap(r => r.highlights.map(h => h.tag)))].filter(t => TAGS[t]);
$('#filters').innerHTML = `<button class="on" data-tag="">Tout</button>` +
  used.map(t => `<button data-tag="${t}">${TAGS[t]}</button>`).join('');

$('#road').insertAdjacentHTML('beforeend', data.releases.map((r, i) => {
  const cast = r.new_brawlers.map(brawler).filter(Boolean);
  return `
  <li class="stop reveal" style="--c:${r.color};--g:${grad(r)}">
    <span class="node"><i>${r.emoji}</i></span>
    <article class="rel">
      <header>
        <span class="ver">${r.version}<small>version ${i + 1} / ${data.releases.length}</small></span>
        <h3>${esc(r.name)}</h3>
        <p class="teaser">« ${esc(r.teaser_fr)} »</p>
        <p class="promise">${esc(r.promise_fr)}</p>
        ${cast.length || r.new_maps.length ? `<div class="newbies">
          ${cast.map(b => `<a class="who" href="#b-${b.image}" style="--bc:${b.color}"><img src="${art(b.image)}" alt="" loading="lazy" /><span><b>${esc(b.name)}</b>${esc(b.role_fr)}</span></a>`).join('')}
          ${r.new_maps.map(m => `<span class="map">🗺️ <span><b>${esc(m)}</b>Nouvelle arène</span></span>`).join('')}
        </div>` : ''}
      </header>
      <ul class="hl">
        ${r.highlights.map(h => `<li data-tag="${h.tag}"><span class="e">${h.emoji}</span><div><b>${esc(h.title_fr)}</b><p>${esc(h.text_fr)}</p><span class="tag">${TAGS[h.tag] || h.tag}</span></div></li>`).join('')}
      </ul>
    </article>
  </li>`;
}).join('') + `
  <li class="stop end reveal"><span class="node"><i>🚀</i></span><a class="now-card" href="#apres"><span class="ver">ensuite</span><b>Et après ?</b><span>Élimination 3 contre 3, la Caldeira, les saisons...</span></a></li>`);

$('#newcards').innerHTML = data.brawlers.map(b => {
  const r = release(b.release);
  return `
  <article class="ncard reveal" id="b-${b.image}" style="--c:${b.color}">
    <div class="art"><img src="${art(b.image)}" alt="${esc(b.name)}, concept" loading="lazy" /></div>
    <span class="when" style="--rc:${r ? r.color : 'rgba(255,255,255,0.25)'}">${r ? `${r.emoji} ${r.version} · ${esc(r.name)}` : '⏳ Plus tard'}</span>
...
$('#later').innerHTML = data.later.map(l => `
  <div class="tile reveal"><span class="ico">${l.emoji}</span><div><h3>${esc(l.title_fr)}</h3><p>${esc(l.text_fr)}</p></div></div>`).join('');
$('#moons').innerHTML = data.moonshots.map(m => `
  <div class="mcard reveal"><span class="ico">${m.emoji}</span><h3>${esc(m.title_fr)}</h3><p>${esc(m.text_fr)}</p></div>`).join('');
...
$('#filters').addEventListener('click', e => {
  ...
  if (!btn) return;
  for (const b of $('#filters').children) b.classList.toggle('on', b === btn);
